...
test("toAirport rejects no-IATA, unscheduled, or heliport rows", () => {
  expect(
    toAirport(row({ iata_code: "", scheduled_service: "yes", type: "large_airport" }), name),
  ).toBeNull();
  expect(
    toAirport(row({ iata_code: "XXX", scheduled_service: "no", type: "large_airport" }), name),
  ).toBeNull();
  expect(
    toAirport(row({ iata_code: "XXX", scheduled_service: "yes", type: "heliport" }), name),
  ).toBeNull();
});
